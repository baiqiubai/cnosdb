...
use spi::query::ast::{
    ...
    DropTenantObject, DropVnode, Explain, ExtStatement, GrantRevoke, MoveVnode, NodeState,
    OutputMode, Privilege, ShowSeries, ShowTagBody, ShowTagValues, Trigger, UriLocation, With,
};
    ...
    fn parse_change_node_state(&mut self) -> Result<ExtStatement> {
        if self.parse_cnos_keyword(CnosKeyWord::NODE) {
            ...
            if !self.parse_cnos_keyword(CnosKeyWord::STATE) {
                return parser_err!("Expected STATE,after NODE ID");
            }
...
mod tests {
    ...
    fn test_change_node_state() {
        ...
        assert_eq!(statements.len(), 1);
        match &statements[0] {
            ExtStatement::ChangeNodeState(ChangeNodeState {
                node_id,
                node_state,
            }) => {
